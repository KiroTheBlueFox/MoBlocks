...
public class CoffeeTable extends Block implements IWaterLoggable {
	 ...
	 */
	private boolean isBlockAtCoffeeTable(String direction, BlockState stateIn, IWorld worldIn, BlockPos currentPos) {
	...
	public BlockState forEachDirections(BlockState stateIn, IWorld worldIn, BlockPos currentPos) {
		...
		allDirections.forEach((direction, property) -> {
			state[0] = state[0].with(property, isBlockAtCoffeeTable(direction, stateIn, worldIn, currentPos));
		});
		return state[0];
	}

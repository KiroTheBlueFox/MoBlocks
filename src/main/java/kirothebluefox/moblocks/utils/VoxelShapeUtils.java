...
public class VoxelShapeUtils {
	public static List<double[]> toListOfCoordinates(VoxelShape voxelShape) {
		...
		List<AxisAlignedBB> bbList = voxelShape.toBoundingBoxList();
		for (int i = 0; i < bbList.size(); i++) {
			AxisAlignedBB bb = bbList.get(i);
			voxelShapeCoordinates.add(new double[] {bb.getMin(Axis.X)*16, bb.getMin(Axis.Y)*16, bb.getMin(Axis.Z)*16, bb.getMax(Axis.X)*16, bb.getMax(Axis.Y)*16, bb.getMax(Axis.Z)*16});
		};
	...
	public static VoxelShape fromListToVoxelShape(List<double[]> voxelShapeCoordinates) {
		VoxelShape fullShape = VoxelShapes.empty();
		for (int i = 0; i < voxelShapeCoordinates.size(); i++) {
			double[] coordinates = voxelShapeCoordinates.get(i);
			fullShape = VoxelShapes.or(fullShape, Block.makeCuboidShape(coordinates[0], coordinates[1], coordinates[2], coordinates[3], coordinates[4], coordinates[5]));
		}
		return fullShape;
	}
	...
	public static VoxelShape rotateYAngle(VoxelShape voxelShape, VoxelShapeUtils.Angle angle) {
		VoxelShape fullShape = VoxelShapes.or(voxelShape);
